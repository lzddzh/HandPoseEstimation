...
Node* Tree::learning(vector<example> &E, const vector<feature> &feats, int depth) {
	int n;
	if (E.size() <= exampleSizeThreshold || depth > depthThreshold || H(E) == -1 * numeric_limits<double>::infinity()) { Node *leaf = new Node(averageLabelValueOfExamples(E), depth); return leaf; }
	else {
		float splitPoint;
